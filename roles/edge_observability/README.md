...
## Keepalived integration (recommended)

Run edge observability only on the active VRRP node.
...
vrrp_instance VI_1 {
  ...
  notify_master "/usr/local/bin/edge-observability-notify master"
  notify_backup "/usr/local/bin/edge-observability-notify backup"
  notify_fault  "/usr/local/bin/edge-observability-notify fault"
}
```

The systemd unit also includes a guard that checks for the VIP before starting.
