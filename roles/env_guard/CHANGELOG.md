...
## [2.1.4] - 2025-09-10
### Changed
- Switched audit logging to per-run folders under `env_guard_logs` to reduce clutter
- Standardised on a `latest` symlink for POSIX systems and a `LATEST_RUN_PATH.txt` marker file on Windows
- Updated test and CI playbooks to copy audit log and report files from the latest run folder into `tests/output`
- Improved dynamic path discovery for audit artefacts
